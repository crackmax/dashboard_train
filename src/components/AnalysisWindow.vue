                ...
                <v-row>
                    <v-col cols="3">
                        <v-select
                            ...
                            label="Select instrument anomaly number"
                            :items="indexedInstAnomalies"
                            no-data-text="no anomalies"
                            @change="createChart"
                        ></v-select>
                    </v-col>
                    <v-col cols="1"> </v-col>
                ...
                <v-row>
                    <v-col cols="3">
                        <v-select
                            ...
                            label="Select column to analyse"
                            :items="columnTitles"
                            no-data-text="no anomalies"
                            @change="createChart"
                        ></v-select>
...
export default {
    ...
    mounted() {
        ...
        eventBus.$on('home', (data) => {
            this.home = data;
        });
    },
    computed: {
        indexedInstAnomalies() {
            return this.inst_anomalies.map((item, index) => ({
                index,
                text: index,
            }));
        },
        selectedAnomalyCenter() {
            ...
            if (this.inst_anomalies && this.inst_anomalies.length > 0) {
                center.push(
                    this.inst_anomalies[this.selectedItem][
                        Math.floor(this.inst_anomalies[0].length / 2)
                    ].lat,
                );
                center.push(
                    this.inst_anomalies[this.selectedItem][
        ...
        createChartData(dataArray) {
            ...
            dataArray.forEach((obj) => {
                ...
                watTempData.push(
                    obj[
                        this.objectTitles[
                            2 * this.columnTitles.indexOf(this.selectedColumn)
                        ]
                    ],
                );
                if (!obj.RS_E_WatTemp_PC2) {
                    ...
                    watTempData_PC2.push(
                        obj[
                            this.objectTitles[
                                2 *
                                    this.columnTitles.indexOf(
                                        this.selectedColumn,
                                    ) +
                                    1
                            ]
